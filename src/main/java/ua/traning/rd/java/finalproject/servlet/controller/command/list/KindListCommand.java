...
public class KindListCommand implements Command {
    ...
    public String execute(HttpServletRequest request) {
        ...
        EntityListService<Kind> kindService = new EntityListService<>(Kind.class);
        try {
            totalRecords = kindService.totalQuantity();
        ...
        if (pageCount > 1) {
            for (int i = 0; i < Math.ceil((double) totalRecords / rowsPerPage); i++) {
                pagesLinks.add(String.format("/topagekind?pagenumber=%d&rowsPerPage=%d", i + 1, rowsPerPage));
            }
        }
